...
class GrowthPhase(Enum):
    ...
    CALIBRATION = "calibration"    # 0-150 trades: Fixed sizing & edge validation
...
class CompoundConfig:
    ...
    calibration_trades_required: int = 150  # Number of trades to validate edge
    calibration_min_win_rate: float = 0.55  # Minimum 55% win rate to graduate
    calibration_fixed_size_sol: float = 0.006  # Fixed $2 trade size (~0.006 SOL)
    calibration_profit_vault_threshold: float = 0.2  # 20% of capital to trigger compound
...
class CompoundMetrics:
    ...
    profit_vault_balance: float = 0.0
...
class HyperCompoundEngine:
    ...
    async def process_trade_profit(self, trade_result: Dict[str, Any]) -> bool:
        """Process profit from a successful trade with anti-fragile vault-based batching"""
        try:
            ...
            if success:
                ...
                if profit_amount > 0:
                    ...
                    # Anti-fragile strategy: Send profits to vault immediately during calibration
                    if self.current_phase == GrowthPhase.CALIBRATION:
                        await self._deposit_to_profit_vault(profit_amount)
                        self.logger.info(f"🏦 Calibration profit secured to vault: {profit_amount:.6f} SOL")
                    else:
                        # Post-calibration: Use vault-based batching instead of immediate compounding
                        await self._deposit_to_profit_vault(profit_amount)
                        self.logger.info(f"🏦 Profit deposited to vault: {profit_amount:.6f} SOL")
    ...
    async def _deposit_to_profit_vault(self, profit_amount: float) -> bool:
        """Deposit profits to the PROFIT_VAULT instead of immediate compounding"""
        try:
            if self.vault_system:
                # Deposit to PROFIT_VAULT
                result = await self.vault_system.deposit_profits(profit_amount)
                if result.get('success', False):
                    self.metrics.profit_vault_balance += profit_amount
                    return True
                else:
                    self.logger.error(f"❌ Failed to deposit {profit_amount:.6f} SOL to profit vault")
                    return False
            else:
                # Fallback: track in metrics if vault system not available
                self.metrics.profit_vault_balance += profit_amount
                return True
                
        except Exception as e:
            self.logger.error(f"❌ Error depositing to profit vault: {e}")
            return False
    ...
    async def _compound_monitoring_loop(self):
        ...
        while self.compounding_active:
            try:
                # Check if we should graduate from calibration
                if self.current_phase == GrowthPhase.CALIBRATION:
                    if await self._should_graduate_from_calibration():
                        await self._graduate_from_calibration()
                else:
                    # Check if we should compound (post-calibration only)
    ...
    async def _should_graduate_from_calibration(self) -> bool:
        """Check if system should graduate from calibration phase"""
        try:
            # Must complete required number of trades
            if self.metrics.trades_completed < self.config.calibration_trades_required:
                return False
            
            # Must achieve minimum win rate
            if self.metrics.trades_completed > 0:
                current_win_rate = self.metrics.successful_trades / self.metrics.trades_completed
                if current_win_rate < self.config.calibration_min_win_rate:
                    self.logger.warning(f"⚠️ Edge validation failed: {current_win_rate:.1%} < {self.config.calibration_min_win_rate:.1%}")
                    return False
                
                self.logger.info(f"✅ Edge validated: {current_win_rate:.1%} win rate over {self.metrics.trades_completed} trades")
                return True
            
            return False
            
        except Exception as e:
            self.logger.error(f"Error checking calibration graduation: {e}")
            return False
    
    async def _graduate_from_calibration(self):
        """Graduate from calibration to bootstrap phase"""
        try:
            self.logger.info("🎓 GRADUATING FROM CALIBRATION PHASE")
            
            # Mark edge as validated
            self.metrics.edge_validated = True
            
            # Transition to bootstrap phase
            self.current_phase = GrowthPhase.BOOTSTRAP
            
            # Reconfigure settings for bootstrap phase
            await self._configure_phase_settings()
            
            self.logger.info(f"🚀 Entered BOOTSTRAP phase with validated edge")
            
        except Exception as e:
            self.logger.error(f"Error graduating from calibration: {e}")
    
    async def _should_compound(self) -> bool:
        """Determine if we should execute a compound cycle (post-calibration only)"""
        try:
            # Never compound during calibration phase
            if self.current_phase == GrowthPhase.CALIBRATION:
                return False
            
            # Check if profit vault has reached threshold
            if self.wallet_manager:
                # Get total active trading capital
                all_wallets = await self.wallet_manager.get_all_wallets()
                total_capital = sum(wallet.get('balance', 0.0) for wallet in all_wallets.values())
                
                # Calculate threshold (20% of active capital)
                threshold = total_capital * self.config.calibration_profit_vault_threshold
                
                if self.metrics.profit_vault_balance >= threshold:
                    self.logger.info(f"💰 Profit vault threshold reached: {self.metrics.profit_vault_balance:.6f} SOL >= {threshold:.6f} SOL")
                    return True
            
            # Check minimum profit threshold as fallback
            if self.metrics.profit_vault_balance < self.config.min_profit_threshold:
                return False
    ...
    async def _execute_compound_cycle(self):
        """Execute a compound cycle using profit vault batching"""
        try:
            self.logger.info("🔄 Executing vault-based compound cycle...")
            
            # Use profit vault balance as source for compounding
            total_profit = self.metrics.profit_vault_balance
            ...
            # Calculate vault allocation (remaining stays in vault)
            ...
            if success:
                # Reset profit vault balance (compound amount was distributed)
                self.metrics.profit_vault_balance = vault_amount
                self.last_compound_time = datetime.now()
                self.metrics.compound_cycles_completed += 1
                
                self.logger.info(f"✅ Vault-based compound cycle complete: {compound_amount:.6f} SOL distributed to wallets")
            else:
