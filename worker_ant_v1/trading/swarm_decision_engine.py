...
class SwarmDecisionEngine:
    ...
    def __init__(self, swarm_id: str = "default"):
        ...
        self.ml_predictor: Optional[PredictionEngine] = None
    ...
    async def _handle_market_opportunity(self, message: MessageEnvelope):
        ...
        try:
            ...
            # Get narrative weight for this token
            narrative_weight = await self._get_narrative_weight_for_token(token_address, opportunity_data)
            
            # Analyze opportunity using neural command center with narrative weighting
            consensus_signal = await self.neural_command_center.analyze_opportunity(
                token_address, opportunity_data, narrative_weight
            )
    ...
    async def _get_narrative_weight_for_token(self, token_address: str, opportunity_data: Dict[str, Any]) -> float:
        """Get narrative weight for a token based on its classification and current dominant narratives"""
        try:
            # Get token metadata for narrative classification
            token_metadata = {
                'name': opportunity_data.get('token_name', ''),
                'symbol': opportunity_data.get('token_symbol', ''),
                'description': opportunity_data.get('description', '')
            }
            
            # Classify token using NarrativeAnt (if neural command center has one)
            if hasattr(self.neural_command_center, 'narrative_ant') and self.neural_command_center.narrative_ant:
                narrative_ant = self.neural_command_center.narrative_ant
                
                # Classify the token's narrative
                token_classification = await narrative_ant.analyze_token_narrative(token_address, token_metadata)
                
                # Get current capital allocation recommendations
                allocation_recommendations = await narrative_ant.get_capital_allocation_recommendations()
                
                # Find the weight for this token's primary narrative
                primary_narrative = token_classification.primary_category
                narrative_data = allocation_recommendations.get(primary_narrative, {})
                allocation_percentage = narrative_data.get('allocation_percentage', 0.0)
                
                # Convert allocation percentage to narrative weight (0.0 to 1.0)
                # Tokens in dead narratives get very low weight, dominant narratives get full weight
                if allocation_percentage <= 0.05:  # Less than 5% allocation = dead narrative
                    narrative_weight = 0.1  # Severely downgrade
                elif allocation_percentage <= 0.15:  # Less than 15% allocation = weak narrative
                    narrative_weight = 0.5  # Moderate downgrade
                else:  # Strong narrative
                    narrative_weight = min(1.0, allocation_percentage * 2.5)  # Scale up to max 1.0
                
                self.logger.debug(f"🏷️ Token {token_address} classified as {primary_narrative.value} "
                               f"(allocation: {allocation_percentage:.1%}, weight: {narrative_weight:.2f})")
                
                return narrative_weight
            else:
                # Fallback: no narrative intelligence available, use neutral weight
                return 1.0
                
        except Exception as e:
            self.logger.error(f"❌ Error getting narrative weight for {token_address}: {e}")
