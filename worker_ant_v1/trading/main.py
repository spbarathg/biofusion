...
class MemecoinTradingBot:
    ...
    def __init__(self, initial_capital: float = 300.0):
        ...
        self.logger.info(f"🚀 Memecoin Trading Bot initialized with ${initial_capital} capital")
    ...
    async def _process_opportunity(self, opportunity: Dict[str, Any]):
        ...
        try:
            ...
            if (sentiment_decision.decision == "BUY" and 
                sentiment_decision.sentiment_score >= self.trading_config['sentiment_threshold'] and
                sentiment_decision.confidence >= 0.6):
    ...
    async def _open_position(self, opportunity: Dict[str, Any], sentiment_decision: Any):
        ...
        try:
            ...
            if result.success:
                ...
                await self.wallet_manager.update_wallet_performance(wallet.wallet_id, {
                    'success': True,
    ...
    def _calculate_position_size(self, opportunity: Dict[str, Any], sentiment_decision: Any) -> float:
        ...
        try:
            ...
            position_size = base_size * sentiment_multiplier * confidence_multiplier * profit_multiplier
    ...
    async def _check_sentiment_exit(self, token_address: str, position: Dict[str, Any]):
        ...
        try:
            ...
            if sentiment_decision.decision == "SELL" and sentiment_decision.confidence >= 0.7:
    ...
    async def _close_position(self, token_address: str, reason: str):
        ...
        try:
            ...
            if result.success:
                ...
                # Update wallet performance
                await self.wallet_manager.update_wallet_performance(wallet_id, {
    ...
    async def _compound_profits(self, profit_sol: float):
        ...
        try:
            ...
            self.logger.info(f"💰 Compounded {compound_amount:.4f} SOL, vaulted {vault_amount:.4f} SOL")
        ...
        while self.trading_active:
            try:
                ...
                if self.stats['trades_executed'] > 0:
                    ...
                    self.stats['avg_profit_per_trade'] = self.stats['total_profit_sol'] / self.stats['trades_executed']
        ...
        while self.trading_active:
            try:
                ...
                if self.current_capital < self.initial_capital * 0.8:  # 20% drawdown
                    self.logger.warning("⚠️ Significant drawdown detected, reducing position sizes")
    ...
    async def _log_performance_summary(self):
        ...
        try:
            ...
            self.logger.info(f"   Capital: ${self.current_capital:.2f} (${self.initial_capital:.2f} initial)")
...
async def main():
    ...
    parser.add_argument('--capital', type=float, default=300.0, help='Initial capital in USD')
    parser.add_argument('--mode', choices=['production', 'simulation'], default='production', help='Trading mode')
